...
def test_errors(**arguments):
    ...
    if "coordinates" in arguments:
        ...
        assert 0 <= coordinates[0] < sudoku_size and 0 <= coordinates[
            1] < sudoku_size, f"The `coordinates` argument must contains integers between 0 and grid_size - 1 (value : {coordinates})"
    ...
    if "list_values" in arguments:
        ...
        for sub_list in list_values:
            ...
            assert len(
                sub_list) == sudoku_size, f"The `list_values` argument must contains lists that have lengths of {sudoku_size} (value : {list_values})"
            
            for value in sub_list:
                assert type(
                    value) == int, f"The `list_values` argument must contains lists that contains integers (value : {list_values})"
                assert 0 <= value <= sudoku_size, f"The `list_values` argument must contains lists that contains integers between 0 and {sudoku_size} (value : {list_values})"
    ...
    if "list_states" in arguments:
        ...
        assert len(
            list_states) == sudoku_size, f"The `list_states` argument must have a length of {sudoku_size} (length : {len(list_states)})"
        
        for sub_list in list_states:
            assert type(sub_list) == list, f"The `list_states` argument must contains lists (value : {list_states})"
            assert len(
                sub_list) == sudoku_size, f"The `list_states` argument must contains lists that have lengths of {sudoku_size} (value : {list_states})"
    ...
    if "grid_size" in arguments:
        ...
        assert (sudoku_size ** 0.5).is_integer() == True, f"The 'grid_size' argument must be a perfect square, x² = 1², 2², 3², 4², ... (value : {sudoku_size})"
